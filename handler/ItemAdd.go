...
func ItemAdd(c *gin.Context) {
	...
	rpcAddItemReq := &product_core.AddItemReq{
		...
		ItemInfo: &product_core.ItemInfo{
			Name:               addItemReq.ItemInfo.Name,
			Description:        addItemReq.ItemInfo.Description,
			Category:           addItemReq.ItemInfo.Category,
			Price:              addItemReq.ItemInfo.Price,
			Stock:              addItemReq.ItemInfo.Stock,
			ImageUrl:           addItemReq.ItemInfo.ImageURL,
			Province:           addItemReq.ItemInfo.Province,
			City:               addItemReq.ItemInfo.City,
			District:           addItemReq.ItemInfo.District,
			UserId:             int32(uid),
			UserType:           tokenInfo.Role,
			SpecificAttributes: addItemReq.ItemInfo.SpecificAttributes,
		},
		TagList: addItemReq.TagList,
	}
